...
class TransformerEncoderPixelDecoder(PixelDecoder):
    ...
    def forward(self, feats, img_metas):
        ...
        feat_last = self.encoder_in_proj(feat_last)  # pass the resent bacbone last feature map through a 1x1 conv to reduce the channel from 2048 to 256
        ...
        # pass the backbone last feature map to a transformer encoder, where it is processed sequenctially through 6 layers of MHA blocks
        # call DetrTransformerEncoder.forward(); # self.encoder = DetrTransformerEncoder
        memory = self.encoder(query=feat_last, key=None, value=None, query_pos=pos_embed, query_key_padding_mask=padding_mask)

        # (num_queries, batch_size, c) -> (batch_size, c, h, w)
        memory = memory.permute(1, 2, 0).view(bs, self.encoder_embed_dims, h, w)

        # memory.shape = torch.Size([1, 256, 20, 19])
        # memory is passed through Conv (3x3), GroupNorm, ReLU
