...
def get_box_mask_pseduo_labels(ema_panoptic_label, dev, batch_size, map_cid_to_pos_index, label_divisor_target):
    '''
    ...
    for bid in range(batch_size):
        ...
        labels = []
        masks = []
        ema_pan = ema_panoptic_label[bid]
        ids = ema_pan.unique()
        height, width = ema_pan.shape
        # print(ids)
        any_valid_box = False
        for id in ids:
            ...
            if isValidBox(box):
                any_valid_box = True
                domainId = 1 # mapId2Domain(id) # 0: source domain, 1: target domain
                masks.append(mask.astype(np.uint8))
                bboxes.append(torch.FloatTensor(box))
                lbl_divisor = label_divisor_target
...
@UDA.register_module()
class DACSInstV2(DACS):
    def __init__(self, **cfg):
        super(DACSInstV2, self).__init__(**cfg)
        ...
        self.switch_off_self_training = cfg['switch_off_self_training'] # NOT IN USE
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, gt_bboxes, gt_labels,
        ...
        if self.activate_uda_inst_losses:
            ...
            with torch.no_grad():
                ...
                ema_bboxes, ema_labels, ema_masks, box_domain_indicator = get_box_mask_pseduo_labels(
                                                                                                        ema_panoptic_label,
                                                                                                        dev, batch_size,
                                                                                                        self.map_cid_to_pos_index,
                                                                                                        self.label_divisor_target,
                                                                                                    )
        ...
        # THIS IS NOT THE CORRECT WAY TO SWTICH OFF SELF TRAINING
        # LATER DURING CVPR 2023 ABLATION I DID THE CORRECT WAY OF SWTICH OF MEAN TEAHCER BY SETTING THE ALPHA TO0
        ...
        mixed_bboxes, mixed_labels, mixed_masks, box_domain_indicator_mix =  None, None, None, None
        # if there is no valid pseduo bbox and masks then set the pseudo_wght_val to None
        pseudo_wght_val_mix = None #  if mixed_bboxes is None else pseudo_wght_val
        ...
        mix_losses = self.get_model().forward_train(
                                                        ...
                                                        box_domain_indicator=box_domain_indicator_mix,
                                                        pseudo_wght_val=pseudo_wght_val_mix,
        ...
        # train on target images for only instance losses
        if ema_bboxes is not None:
            target_inst_losses = self.get_model().forward_train(
                                                                    target_img,
                                                                    target_img_metas,
                                                                    None,
                                                                    seg_weight=None,
                                                                    return_feat=True,
                                                                    gt_bboxes=ema_bboxes,
                                                                    gt_labels=ema_labels,
                                                                    gt_masks=ema_masks,
                                                                    box_domain_indicator=None,
                                                                    pseudo_wght_val=None,
                                                                    activate_visual_debug=activate_visual_debug,
                                                                    use_instance_pseduo_losses=True,
                                                                )
            target_inst_losses.pop('features')
            target_inst_losses = add_prefix(target_inst_losses, 'trg_inst')
            target_inst_loss, target_inst_log_vars = self._parse_losses(target_inst_losses)
            log_vars.update(target_inst_log_vars)
            target_inst_loss.backward()
